...
inline float triArea(vec2 A, vec2 B, vec2 C){
    float resultado = ((C.x - A.x)*(B.y - A.y)/ 2) - ((B.x - A.x)*(C.y - A.y)/2);
    return resultado;
}


inline std::array<float, 3> barycentric(vec2 Ponto, std::array<vec2, 3> Triangulo) {
    std::array<float, 3> resultado;
    float areaTotal = triArea(Triangulo[0],Triangulo[1],Triangulo[2]);
    resultado[0] = triArea (Ponto, Triangulo[1],Triangulo[2])/ areaTotal; ///alfa 1
    resultado[1] = triArea (Triangulo[0],Ponto, Triangulo[2])/ areaTotal; ///alfa 2
    resultado[2] = triArea (Triangulo[0],Triangulo[1],Ponto)/ areaTotal;  ///alfa 3

    return resultado;
}

inline float distanciaP2 (float p1X, float p1Y, float p2X, float p2Y){
